...
ogr2ogr -f "PostgreSQL" PG:"dbname=$POSTGRES_DB user=$POSTGRES_USER" "data/OpenDataDC_Intersection_Points_for_accuracy_metric.geojson" -nln "public.opendata_dc_intersections" -append
psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "create index oddc_point_index on public.opendata_dc_intersections using gist(wkb_geometry);"
...
psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "GRANT SELECT ON table public.opendata_dc_intersections TO ${POSTGRES_BUILDER_USER}"
