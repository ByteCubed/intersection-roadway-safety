...
def extract_intersection_oneway(G: networkx.MultiDiGraph, node):
    ...
    Returns
    -------
    Proportion of roadway legs that are oneway 0<=p<=1
    '''
    count = 0
    ...
    for n1, n2 in G.edges(node):
        ...
        for data in edge_data.values():
            if not 'oneway' in data:
                ...
                if np.isnan(data['oneway']):
                    continue
                else:
                    if data['oneway']:
                        count += 1

    return count / len(G.edges(node))
...
def extract_intersection_angles(G: networkx.MultiDiGraph, node):
    ...
    if len(bearings) > 1:
        return IntersectingAngle(abs(bearings[0] - bearings[1]))
