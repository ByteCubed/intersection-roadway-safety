-- public.waylet definition

-- Drop table

-- DROP TABLE public.waylet;

CREATE TABLE if not exists public.waylet (
...
CREATE INDEX if not exists waylet_line_idx ON public.waylet USING gist (waylet_line);
...
where (wn1.node_order = wn2.node_order + 1 or (w.closed and wn1.node_order=1 and wn2.node_order = array_upper(w.nodes, 1)))
