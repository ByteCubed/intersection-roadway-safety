...
CREATE TABLE if not exists public.way (
...
from (select id, nodes, tags, generate_subscripts(nodes, 1) as node_index from public.planet_osm_ways pow) as foo
...
update way w set
is_road = subquery.highway not in ('', 'footway', 'bridleway', 'steps', 'cycleway'),
