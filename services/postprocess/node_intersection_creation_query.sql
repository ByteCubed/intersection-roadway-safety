-- node_intersections2 definition

-- Drop table

-- DROP TABLE node_intersections;

CREATE TABLE if not exists node_intersections (
...
array_agg(distinct subquery.way_id) as way_ids, count(distinct subquery.way_id) as associatedwayscount, count(distinct subquery.waylethash) as legs,
...
from (
select n.node_id, n.point3857, CONCAT(GREATEST(w.node_id1, w.node_id2)::text, LEAST(w.node_id1, w.node_id2)::text) as waylethash, w.way_id, w.waylet_line, w.is_road, w.is_railway, w.is_bikepath_or_trail, w.is_footway, cast(n.lat_int as float) / 10000000 as lat, cast(n.long_int as float) / 10000000 as long
from node n
...
CREATE INDEX if not exists node_intersections_index ON node_intersections USING gist (point3857);
